...
void ReadMCF(FILE* File_Ptr, InputStruct* In_Ptr)
{
	char buf[STRLEN];
	strcpy(buf, FindDataLine(File_Ptr));
	if (buf[0] == '\0')
...
Boolean ReadOneLayer(FILE* File_Ptr,
	...
	double* Z_Ptr)
{
	char buf[512], msg[STRLEN];
	double d, n, mua, mus, g, RCS=0;	/* d is thickness. */

	strcpy(buf, FindDataLine(File_Ptr));
	if (buf[0] == '\0') return(1);	/* error. */

	sscanf(buf, "%lf%lf%lf%lf%lf%lf", &n, &mua, &mus, &g, &d, &RCS);
	if (d < 0 || n <= 0 || mua < 0 || mus < 0 || g < 0 || g>1 || RCS<0)
		return(1);			/* error. */
 ...
 ****/
void InitEmit(circle_ini* cir_ini)
{
	cir_ini->x_r = AllocVector_mine(nums_photon_global);
	cir_ini->y_r = AllocVector_mine(nums_photon_global);
}
	...
	OutStruct* Out_Ptr)
{
	...
	Out_Ptr->SO_rz = AllocMatrix(0, nr - 1, 0, nz - 1);
	Out_Ptr->SO_l = AllocVector(0, nl + 1);
	Out_Ptr->SO_z = AllocVector(0, nz - 1);
 ...
 ****/
void FreeData(InputStruct In_Parm, OutStruct* Out_Ptr,circle_ini* ini_ptr)
{
	short nz = In_Parm.nz;
	short nr = In_Parm.nr;
	short na = In_Parm.na;
	...
	FreeMatrix(Out_Ptr->SO_rz, 0, nr - 1, 0, nz - 1);
	FreeVector(Out_Ptr->SO_l, 0, nl + 1);
	FreeVector(Out_Ptr->SO_z, 0, nz - 1);
	...
	FreeVector_mine(ini_ptr->x_r);
	FreeVector_mine(ini_ptr->y_r);
...
void Sum2DSO(InputStruct In_Parm, OutStruct* Out_Ptr)
{
	short nr = In_Parm.nr;
	short nz = In_Parm.nz;
	...
	for (iz = 0; iz < nz; iz++) {
		sum = 0.0;
		for (ir = 0; ir < nr; ir++) sum += Out_Ptr->SO_rz[ir][iz];
		Out_Ptr->SO_z[iz] = sum;
	}
	...
	for (iz = 0; iz < nz; iz++) {
		sum += Out_Ptr->SO_z[iz];
		Out_Ptr->SO_l[IzToLayer(iz, In_Parm)]
			+= Out_Ptr->SO_z[iz];
	}
...
void ScaleA(InputStruct In_Parm, OutStruct* Out_Ptr)
{
	...
	for (iz = 0; iz < nz; iz++)
	{
		for (ir = 0; ir < nr; ir++)
		{
			...
			Out_Ptr->SO_rz[ir][iz] /= (ir + 0.5) * scale1;
		}
	}
...
void WriteInParm(FILE* file, InputStruct In_Parm)
{
	...
	fprintf(file,
		...
		In_Parm.num_layers);
	fprintf(file,
		"#n\tmua\tmus\tg\td\tRCS\t# One line for each layer\n");
	fprintf(file,
	...
	for (i = 1; i <= In_Parm.num_layers; i++) {
		...
		fprintf(file, "%G\t%G\t%G\t%G\t%G\t%G\t# layer %hd\n",
			s.n, s.mua, s.mus, s.g,s.z1 - s.z0,s.RCS, i);
	}
	...
	OutStruct Out_Parm)
{
	...
	fprintf(file,
		"SO_l #Absorption as a function of layer. [-]\n");
	/* flag. */
	...
	OutStruct Out_Parm)
{
	...
	fprintf(file,
		...
		"# SO[1][0], [1][1],..[1][nz-1]",
		"# ...",
		"# SO[nr-1][0], [nr-1][1],..[nr-1][nz-1]",
		"SO_rz");

	for (ir = 0; ir < Nr; ir++)
		for (iz = 0; iz < Nz; iz++) {
			fprintf(file, "%12.4E ", Out_Parm.SO_rz[ir][iz]);
			if ((ir * Nz + iz + 1) % 5 == 0) fprintf(file, "\n");
		}
	fprintf(file, "\n");
...
void WriteMatrix(InputStruct In_Ptr,
	OutStruct Out_Ptr)
{
	FILE* file;
	char        fname[STRLEN];
	...
	sprintf(fname, "SORZ.txt");
	file = fopen("SORZ.txt", "w");
	if (file == NULL)
		return;

	for (ir = 0; ir < nr; ir++)
		for (iz = 0; iz < nz; iz++) {
			fprintf(file, "%-12.4E ", Out_Ptr.SO_rz[ir][iz]);
			if (iz == nz - 1) { fprintf(file, "\n"); }
	...
	char* TimeReport)
{
	...
	strftime(name, sizeof(name), "%Y-%m-%d-%H-%M-%S.mco", localtime(&timep)); //最后一个参数是用localtime将秒数转化为struct tm结构体

	printf("创建文件名称为：%s\n", name);
	/*
	file = fopen(In_Parm.out_fname, "wb+");*/
	file = fopen(name, "wb+");
  ...
  /* 1D arrays. */
	...
	WriteSO_layer(file, In_Parm.num_layers, Out_Parm);
	WriteA_z(file, In_Parm.nz, Out_Parm);
	WriteSO_z(file, In_Parm.nz, Out_Parm);
	...
	WriteSO_rz(file, In_Parm.nr, In_Parm.nz, Out_Parm);
