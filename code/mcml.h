...
typedef struct {
	...
	/**********************/
	//加了功能
	short raman_layer;	/* depth where Raman scattering occurs*/
	
	Boolean is_Raman;
	double of_tracex_start;
	double of_tracey_start;
	double of_tracez_start;
	double of_tracex_end;
	double of_tracey_end;
	double of_tracez_end;
...
/****
 *	Structure used to describe the geometry(几何学) and optical
 *	properties of a layer.
 *	z0（上边缘） and z1（下边缘） are the z coordinates for the upper boundary
 ...
 *
 *	z and r are for the cylindrical coordinate system（圆柱坐标系）. [cm]
 *	a is for the angle alpha between the photon exiting
 *	direction and the surface normal（表面法线）. [radian]
 *
 *	The grid line separations in z, r, and alpha
 *	￥directions￥ are dz, dr, and da respectively.  
 *	The ￥numbers of grid lines￥ in z, r, and alpha directions are
