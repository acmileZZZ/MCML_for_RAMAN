...
extern  int  raman_count;
void Write_ramanxy_first(PhotonStruct* photo)
{
    
    double      xs, ys, zs,xd,yd,zd,l_weight;
    FILE* file;
    char        fname[STRLEN] = {0};
    ...
    {
    case(1):
        sprintf(fname, "Axis-r-v1-%s.txt",name);
        break;
    case(2):
        sprintf(fname, "Axis-r-v2-%s.txt", name);
        break;
    case(3):
        sprintf(fname, "Axis-r-v3-%s.txt", name);
        break;
    case(4):
        sprintf(fname, "raw-%s.txt", name);
        break;
    case(5):
        sprintf(fname, "splayer-%s.txt", name);
        break;
    }
    file = fopen(fname, "a+");
    if (file == NULL)
        return;
    //fprintf(file, "%-12s\t%-12s\t%-12s\t%-12s\t%-12s\t%-12s\n", "weight", "x_start[cm]", "y_start[cm]", "z_start[cm]", "x_end[cm]", "y_end[cm]");
    if (mode_shift == 0)
    {
        
        xs = photo->of_tracex_start;
        ys = photo->of_tracey_start;
        zs = photo->of_tracez_start;
        xd = photo->of_tracex_end;
        yd = photo->of_tracey_end;
        l_weight = photo->local_weirht;
        fprintf(file, "%-12.4E\t%-12.4E\t%-12.4E\t%-12.4E\t%-12.4E\t%-12.4E\n", l_weight, xs, ys, zs, xd, yd);
...
void Write_ramanxy_add(PhotonStruct* photo)
{
 ...
 ****/
void set_photon_init_position(PhotonStruct* Photon_Ptr, InputStruct* In_Parm)
{
 
    double sigma_r = 0.25 * In_Parm->beam_d_rho;
    double R = sqrt(2.0 * sigma_r * sigma_r * log(1.0 / (1.0 - hybrid_Taus(photon.seed))));
    double theta = 2.0 * PI * hybrid_Taus(photon.seed);
    photon.r.x = R * cos(theta);
    photon.r.y = R * sin(theta);
    photon.r.z = 0;
}

void set_photon_init_focusing(PhotonStruct* Photon_Ptr, InputStruct* In_Parm)
{
    /* Currently this information is stored for each individual photon
       ...
       future.																				*/
    Photon_Ptr->z_f = In_Parm->focal_depth;
    Photon_Ptr->z_R = 0.001 * 4.0 * In_Parm->beam_wavelength * In_Parm->focal_length * In_Parm->focal_length / (PI * In_Parm->beam_d_rho * In_Parm->beam_d_rho);
}

void set_photon_init_velocity(PhotonStruct* Photon_Ptr, InputStruct* In_Parm)
{
    double angle;
    printf("input the angle of Gussian:\n");
    scanf("%lf", &angle);
    double sigma_a = 0.5 * angle / 180 * PI;
    double R = sqrt(2.0 * sigma_a * sigma_a * log(1.0 / (1.0 - hybrid_Taus(photon.seed))));
    double theta = 2.0 * PI * hybrid_Taus(photon.seed);
    photon.v.x = cos(R) * cos(theta) * cos(theta);
    photon.v.y = cos(R) * cos(theta) * sin(theta);
    photon.v.z = cos(R) * sin(theta);
}

void LaunchPhoton(double Rspecular,LayerStruct* Layerspecs_Ptr,PhotonStruct* Photon_Ptr,
    circle_ini* cir_ini, InputStruct* In_Parm)
{
    

    Photon_Ptr->w = 1.0 - Rspecular;
    Photon_Ptr->local_weirht = 0;
    Photon_Ptr->dead = 0;
    Photon_Ptr->is_Raman = 0;
    //Photon_Ptr->layer = 1; layer 的修改
    Photon_Ptr->s = 0;
    Photon_Ptr->sleft = 0;
    Photon_Ptr->trace_count = 0;
    Photon_Ptr->this_layer = 0;
    //Photon_Ptr->R_step = 0;
    Photon_Ptr->of_tracex_end = 0;
    Photon_Ptr->of_tracey_end = 0;
    Photon_Ptr->of_tracez_end = 0;
    Photon_Ptr->of_tracex_start = 0;
    Photon_Ptr->of_tracey_start = 0;
    Photon_Ptr->of_tracez_start = 0;

    //Photon_Ptr->x = cir_ini->x_r[cir_ini->give_count] * 0.001; 我为什么要*0.001
    //Photon_Ptr->y = cir_ini->y_r[cir_ini->give_count] * 0.001;
    ...
    if (version != 5)
    {
        ...
        Photon_Ptr->ux = 0.0;
        Photon_Ptr->uy = 0.0;
        Photon_Ptr->x = cir_ini->x_r[cir_ini->give_count] ;
        Photon_Ptr->y = cir_ini->y_r[cir_ini->give_count] ;
    }
    else
    {
        //Photon_Ptr->x = cir_ini->x_r[cir_ini->give_count] * 0.00001;
        //Photon_Ptr->y = cir_ini->y_r[cir_ini->give_count] * 0.00001;
        set_photon_init_position(Photon_Ptr, In_Parm);
        set_photon_init_focusing(Photon_Ptr, In_Parm);
        set_photon_init_velocity(Photon_Ptr, In_Parm);
    }


    //Photon_Ptr->uz = 1.0;
    photon_count++;
    if (isinf(Photon_Ptr->x))
    {
        printf("go wrong");
        system("pause");
    }
 ...
 ****/
void RecordSO(double			Refl,	/* reflectance. */
    InputStruct* In_Ptr,
    PhotonStruct* Photon_Ptr,
    OutStruct* Out_Ptr)
{
    double x = Photon_Ptr->of_tracex_end;
    double y = Photon_Ptr->of_tracey_end;

    short  ir, iz;
    ir = (sqrt(x * x + y * y) / In_Ptr->dr);
    if (ir > In_Ptr->nr - 1) ir = In_Ptr->nr - 1;

    iz = (Photon_Ptr->of_tracez_start / In_Ptr->dz);
    if (iz > In_Ptr->nz - 1) iz = In_Ptr->nz - 1;
    Photon_Ptr->local_weirht = Photon_Ptr->w;

    //Out_Ptr->SO_rz[ir][iz] += Photon_Ptr->w;//??????
    printf("\n************************我要写了哦*************************\n");
    if (axis_time == 0)
    {
        Write_ramanxy_first(Photon_Ptr);
        axis_time += 1;
    }
    else
    {
        Write_ramanxy_add(Photon_Ptr);
    }
    ...
    InputStruct* In_Ptr, OutStruct* Out_Ptr)
{
    ...
    //每走一步判断是否变成拉曼光子
     ...
     if (version != 5) {
         if ((a < In_Ptr->layerspecs[layer].RCS) && (Photon_Ptr->is_Raman == 0)
             && (Photon_Ptr->sleft == 0) && (Photon_Ptr->dead == 0))//&& (Photon_Ptr->R_step != 0)
         {
             Photon_Ptr->is_Raman = 1;
             Photon_Ptr->of_tracex_start = Photon_Ptr->x;
             Photon_Ptr->of_tracey_start = Photon_Ptr->y;
             Photon_Ptr->of_tracez_start = Photon_Ptr->z;
             Photon_Ptr->this_layer = 1;//追踪g=0
             raman_count += 1;
             Photon_Ptr->trace_count += 1;
             if (raman_count > nums_photon_global)
             {
                 printf("biggger than total");
                 system("pause");
             }
         }
     }
    ...
    if (Photon_Ptr->sleft == 0.0) {  /* make a new step. */
        ...
        Photon_Ptr->s = -log(rnd) / (mua + mus);//s=0-正无穷
    }
    ...
    OutStruct* Out_Ptr)
{
    ...
    ia = (acos(-Photon_Ptr->uz) / In_Ptr->da);
    ...
    OutStruct* Out_Ptr)
{
    ...
    double uz1=0;	/* cosines of transmission alpha. always */
                  /* positive. */
    ...
    if (layer == 1 && r < 1.0) {	/* partially transmitted. */
        ...
        Photon_Ptr->of_tracex_end = Photon_Ptr->x;
        Photon_Ptr->of_tracey_end = Photon_Ptr->y;
        Photon_Ptr->of_tracez_end = Photon_Ptr->z;
        if (Photon_Ptr->is_Raman == 1)
        {
            RecordSO(0.0, In_Ptr, Photon_Ptr, Out_Ptr);
        }
    ...
    if (RandomNum() > r) {		/* transmitted to layer-1. */
        if (layer == 1) {
            ...
            Photon_Ptr->of_tracey_end = Photon_Ptr->y;
            Photon_Ptr->of_tracez_end = Photon_Ptr->z;
            if (Photon_Ptr->is_Raman == 1 && Photon_Ptr->z==0)
            {
                RecordSO(0.0, In_Ptr, Photon_Ptr, Out_Ptr);
            }
            else 
